...
describe('Fase 1 - Rotas Aninhadas', () => {
    ...
    test('Deve fazer match em rota com 3 níveis de parâmetros', async () => {
        const request = new Request('http://localhost/api/users/123/posts/456/comments/789');
        const app = new RouterWorkers(request);
        
        let capturedParams: any = null;
        await app.get('/api/users/:userId/posts/:postId/comments/:commentId', (req: Req, res: Res) => {
            capturedParams = req.params;
            res.send(req.params);
        });
    ...
    test('Não deve fazer match em rota diferente', async () => {
        ...
        let wasCalled = false;
        await app.get('/api/users/:userId/comments/:commentId', (req: Req, res: Res) => {
            wasCalled = true;
            res.send({ success: true });
        });
...
describe('Fase 1 - Tratamento de Erros', () => {
    ...
    test('Deve capturar erro em middleware', async () => {
        ...
        const failingMiddleware = (req: Req, res: Res) => {
