...
describe('Fase 3 - Route Groups', () => {
  ...
  test('Deve aplicar middlewares do grupo', async () => {
    ...
    const middleware = async (_req: Req, _res: Res) => {
      middlewareCalled = true;
    };

    await group(
      app,
      { prefix: '/api', middlewares: [middleware] },
      async (api) => {
        await api.get('/users', (req: Req, res: Res) => {
          res.ok({ users: [] });
        });
  ...
  test('Deve combinar middlewares de grupos aninhados', async () => {
    ...
    const apiMiddleware = async (_req: Req, _res: Res) => {
      apiMiddlewareCalled = true;
    };

    const v1Middleware = async (_req: Req, _res: Res) => {
      v1MiddlewareCalled = true;
    };

    await group(
      app,
      { prefix: '/api', middlewares: [apiMiddleware] },
      async (api) => {
        await api.group(
          { prefix: '/v1', middlewares: [v1Middleware] },
          async (v1) => {
            await v1.get('/users', (req: Req, res: Res) => {
              res.ok({ users: [] });
            });
  ...
  test('Deve suportar todos os métodos HTTP', async () => {
    const app1 = new RouterWorkers(
      new Request('http://localhost/api/users', { method: 'GET' }),
    );
    const app2 = new RouterWorkers(
      new Request('http://localhost/api/users', { method: 'POST' }),
    );
    const app3 = new RouterWorkers(
      new Request('http://localhost/api/users', { method: 'PUT' }),
    );
    const app4 = new RouterWorkers(
  ...
  test('Deve capturar erro em middleware de grupo', async () => {
    const request = new Request('http://localhost/api/health');
    const app = new RouterWorkers(request);

    let errorCaptured = false;
    const failingMiddleware = async (_req: Req, _res: Res) => {
      throw new Error('Group middleware error');
    };

    // Define o error handler global
    app.onError((err, _req, res) => {
      errorCaptured = true;
      res.serverError(err.message);
    });

    // Define o grupo com o middleware que falha
    await group(
      app,
      { prefix: '/api', middlewares: [failingMiddleware] },
      async (api) => {
        await api.get('/health', (_req: Req, res: Res) => {
          // Este handler não deve ser alcançado
          res.ok({ status: 'ok' });
        });
      },
    );
    ...
    expect(response.status).toBe(500);
    const data = await response.json();
    expect(data.error).toBe('Group middleware error');
  });

  test('Deve funcionar com grupo sem prefixo', async () => {
    const request = new Request('http://localhost/status');
    const app = new RouterWorkers(request);

    // Grupo sem prefixo, apenas para aplicar middlewares, por exemplo
    await group(app, {}, async (root) => {
      await root.get('/status', (_req: Req, res: Res) => {
        res.ok({ status: 'ok' });
      });
    });
